...
const baseDir = 'c:\\Projects\\vaagaacademy';
...
let moved = { reports: 0, sql: 0, assets: 0, scripts: 0 };
let notFound = [];

Object.entries(filesToMove).forEach(([destDir, files]) => {
  files.forEach(filename => {
    const srcPath = path.join(baseDir, filename);
    const destPath = path.join(baseDir, destDir, filename);
    
    if (fs.existsSync(srcPath)) {
      try {
        fs.renameSync(srcPath, destPath);
        console.log(`Moved: ${filename} -> ${destDir}/`);
        if (destDir.includes('reports')) moved.reports++;
        else if (destDir.includes('sql')) moved.sql++;
        else if (destDir.includes('assets')) moved.assets++;
        else if (destDir.includes('scripts')) moved.scripts++;
      } catch (err) {
...
console.log('\n========== SUMMARY ==========');
console.log(`Reports moved: ${moved.reports}`);
console.log(`SQL files moved: ${moved.sql}`);
console.log(`Assets moved: ${moved.assets}`);
console.log(`Scripts moved: ${moved.scripts}`);
console.log(`Total moved: ${moved.reports + moved.sql + moved.assets + moved.scripts}`);
console.log(`Files not found (skipped): ${notFound.length}`);
...
console.log('\nCleanup: removed _organize_files.js');
