...
foreach ($files_to_move as $dest_dir => $files) {
    foreach ($files as $filename) {
        $src = $base_dir . '\\' . $filename;
        $dest = $base_dir . '\\' . str_replace('/', '\\', $dest_dir) . '\\' . $filename;
        if (file_exists($src)) {
            if (rename($src, $dest)) {
                echo "Moved: $filename -> $dest_dir/\n";
                if (strpos($dest_dir, 'reports') !== false) $moved['reports']++;
                elseif (strpos($dest_dir, 'sql') !== false) $moved['sql']++;
                elseif (strpos($dest_dir, 'assets') !== false) $moved['assets']++;
                elseif (strpos($dest_dir, 'scripts') !== false) $moved['scripts']++;
            } else {
...
echo "\n========== SUMMARY ==========\n";
echo "Reports moved: {$moved['reports']}\n";
echo "SQL files moved: {$moved['sql']}\n";
echo "Assets moved: {$moved['assets']}\n";
echo "Scripts moved: {$moved['scripts']}\n";
echo "Total moved: " . array_sum($moved) . "\n";
echo "Files not found (skipped): " . count($not_found) . "\n";
...
unlink(__FILE__);
echo "\nCleanup: removed _organize_files.php\n";
