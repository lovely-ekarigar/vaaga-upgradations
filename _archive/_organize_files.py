...
base_dir = r'c:\Projects\vaagaacademy'
...
files_to_move = {
    '_archive/reports': [
        ...
        'PROJECT_AUDIT_REPORT.md', 'PROJECT_COMPARISON_REPORT.md', 'PROJECT_FIXES_SUMMARY.md',
...
for dest_dir, files in files_to_move.items():
    for filename in files:
        ...
        dest = os.path.join(base_dir, dest_dir, filename)
        if os.path.exists(src):
            try:
                shutil.move(src, dest)
                print(f"Moved: {filename} -> {dest_dir}/")
                if 'reports' in dest_dir:
                    moved['reports'] += 1
                elif 'sql' in dest_dir:
                    moved['sql'] += 1
                elif 'assets' in dest_dir:
                    moved['assets'] += 1
                elif 'scripts' in dest_dir:
                    moved['scripts'] += 1
...
print('\n========== SUMMARY ==========')
print(f"Reports moved: {moved['reports']}")
print(f"SQL files moved: {moved['sql']}")
print(f"Assets moved: {moved['assets']}")
print(f"Scripts moved: {moved['scripts']}")
print(f"Total moved: {sum(moved.values())}")
print(f"Files not found (skipped): {len(not_found)}")
if not_found:
    print('\nNot found:')
    for f in not_found:
        print(f"  - {f}")
...
os.remove(__file__)
print('\nCleanup: removed _organize_files.py')
