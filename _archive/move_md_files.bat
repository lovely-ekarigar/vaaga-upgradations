...
REM Create the _archive\reports directory
if not exist "_archive\reports" (
    mkdir "_archive\reports"
    echo Created directory: _archive\reports
) else (
    echo Directory already exists: _archive\reports
)
...
for %%F in (
    ...
    if exist "%%~F" (
        move "%%~F" "_archive\reports\" >nul 2>&1
        if !errorlevel! equ 0 (
            echo [OK] Moved: %%~F
            set /a moved+=1
        ) else (
            echo [FAIL] Could not move: %%~F
            set /a failed+=1
        )
    ) else (
        echo [NOT FOUND] %%~F
