...
@section('title', 'Edit Test | ' . env('APP_NAME'))
    ...
    <div class="page-content">
        ...
        <div class="card shadow-sm border-0">
            <div class="card-header d-flex justify-content-between align-items-center py-2">
                <h6 class="mb-0">Edit Test</h6>
                <a href="{{ route('admin.testseries.testlist', $test->test_series_id) }}" class="btn btn-secondary btn-sm">
                    <i class="fas fa-arrow-left"></i> Back
                </a>
            </div>

            <div class="card-body">
                <form action="{{ route('admin.testseries.update-test', $test->id) }}" method="POST" class="row g-3">
                    @csrf
                    @method('PUT')
                    ...
                    <div class="col-md-3">
                        ...
                        <input type="number" class="form-control" id="total_questions" name="total_questions" 
                               value="{{ old('total_questions', $test->total_questions) }}" min="1" required>
                    </div>
                    ...
                    <div class="col-md-3">
                        ...
                        <input type="number" class="form-control" id="duration" name="duration" 
                               value="{{ old('duration', $test->duration) }}" min="1" required>
                    </div>

                    <!-- Description -->
                    <div class="col-md-12">
                        <label for="description" class="form-label">Description</label>
                        <textarea class="form-control" id="description" name="description" rows="2">{{ old('description', $test->description) }}</textarea>
                    </div>
                    ...
                    <div class="col-md-2">
                        ...
                        <select name="is_prev_year" id="is_prev_year" class="form-control form-select" required>
                            <option value="0" @if($test->is_prev_year=='0') selected @endif>No</option>
                            <option value="1" @if($test->is_prev_year=='1') selected @endif>Yes</option>
                        </select>
                    </div>
