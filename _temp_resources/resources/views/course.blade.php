...
<script type="application/ld+json">
   {
     ...
     "@type": "Course",
     "name": "@if($bd) {{$bd->name}} - @endif @if($pcategory) {{$pcategory->name}} @else {{$category->name}} @endif - {{$course->title}} | {{env('APP_NAME')}}",
     "description": "VaaGa Academy's online @if($bd) {{$bd->name}} - @endif @if($pcategory) {{$pcategory->name}} @else {{$category->name}} @endif - {{$course->title}} Coaching provides a dynamic education, merging CBSE curriculum with engaging teaching methods for effective learning.",
     "provider": {
       "@type": "Organization",
       "name": "VaaGa Academy",
       "sameAs": "https://www.vaagaacademy.com/courses/{{$course->slug}}"
     }
   }
</script>
<main>
