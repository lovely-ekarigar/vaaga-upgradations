...
class BlogController extends Controller
{
    ...
    public function getByCategory(Request $request)
    {
        ...
        $category = BlogCategory::where('slug', '=', str_slug($request->category))->first();
        $categories = BlogCategory::has('blogs')->paginate(10);
        if ($category != "") {
    ...
    {
        $popular_tags = Tag::has('blogs', '>', 4)->get();
        $categories = BlogCategory::take(10)->get();
  $category =null;
        if ($request->slug != "") {
        ...
        //   dd($next);
            ...
            return view($this->path.'.blogs.blog-single', compact('blog','previous','next','popular_tags','categories'));
        }


        $blogs = Blog::has('category')->OrderBy('created_at','desc');
        
        if($request->k){
        $blogs->where("title","like","%".$request->k."%");
    }
        $blogs = $blogs->paginate(10);
        
        return view($this->path.'.blogs.index',
            compact( 'blogs', 'categories', 'popular_tags','category'));
    }
