...
<div class="">
    <div class="max-w-6xl mx-auto px-4">
        ...
        <div class="card bg-white mb-6">
            ...
            <div class="card-body">
                {{-- FIX: Add explicit route for update --}}
                <form method="POST" action="{{ route('admin.exams.questions.update', $question->id) }}" id="questionForm">
                    @csrf
                    @method('PUT')
...
<select name="verification_status" class="form-control form-select">
<option value="pending" {{ old('verification_status', $question->verification_status) == 'pending' ? 'selected' : '' }}>Pending</option>

<option value="approved" {{ old('verification_status', $question->verification_status) == 'approved' ? 'selected' : '' }}>Approved</option>

<option value="rejected" {{ old('verification_status', $question->verification_status) == 'rejected' ? 'selected' : '' }}>Rejected</option>
</select>
                    ...
                    <div class="d-flex gap-2 sticky-bottom py-3 bg-light mt-4 rounded-2">
                        ...
                        <a href="{{ url()->previous() }}" class="btn btn-light px-4 ms-auto">Cancel</a>
                    </div>
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        ...
        document.querySelectorAll('textarea[id^="option_"][id$="_en"]').forEach(function(textarea) {
            if (textarea.id) CKEDITOR.replace(textarea.id, { height: 150 });
        });
