...
@section('title', "Question Bank | " . env('APP_NAME'))
        ...
        <div class="card shadow-sm border-0">
            ...
            <div class="card-body">
                ...
                <div style="width:50%;" class="mb-4">
                    <div class="input-group">
                        <input type="text" placeholder="Enter search term" class="form-control" id="key" value="{{request('key')}}" />
                        <div class="input-group-append">
                ...
                <div class="table-responsive">
                    <table class="table table-hover align-middle dtable">
                        ...
                        <tbody>
                            ...
                            <tr>
                                {{-- data-question-id for easier selection --}}
                                <td data-question-id="{{ $question->id }}"><strong>#{{ $question->id }}</strong></td>
                                <td>
                                    {{-- FIX: Use full namespace for Str --}}
                                    {{ \Illuminate\Support\Str::limit(strip_tags(json_decode($question->question_text,true)['en'] ?? 'No question text'), 60) }}
                                </td>
                                <td>
                                    <span class="badge 
                                ...
                                <td>
                                    <select class="form-control form-select selectChapter" data-id="{{$question->id}}" style="width:100%">
                                        @php $chapters = App\Models\Lesson::where("course_id",$question->course_id)->get(); @endphp
                                        <option value="">Select Chapter</option>
                                        @foreach($chapters as $c)
                                            <option value="{{$c->id}}" @if($c->id==$question->chapter_id) selected @endif>
                                                {{-- FIX: Use full namespace for Str --}}
                                                {{ \Illuminate\Support\Str::limit($c->title, 30) }}
                                            </option>
                                        @endforeach
                                    </select>
                                </td>
...
<script>
    $(document).ready(function () {
        ...
        function setupFilterAutoSubmit() {
            ...
            $('input[name="search"]').on('keypress', function(e) {
                if (e.which === 13) $('#filterForm').submit();
            });
        }
        ...
        function loadPendingQuestions(courseId = null) {
            let url = "{{ route('admin.exams.questions.pending') }}?course_id={{request('subject_id')}}";
            $.get(url, function(response) {
                pendingQuestions = response.questions || [];
                $('#pendingCount').text(pendingQuestions.length);
                $('#approvedCount').text(response.approved_count || 0);
                $('#rejectedCount').text(response.rejected_count || 0);
                ...
                if (pendingQuestions.length > 0) {
                    ...
                    $('#verifyQuestionsBtn').prop('disabled', true)
                        .html('<i class="fas fa-check-circle"></i> Verify Questions (0)');
                }
            }).fail(() => showAlert('danger', 'Error loading pending questions.'));
        }
                    ...
                    $.get("{{ url('user/chapters/by-subject') }}/" + subjectId, function (data) {
                        ...
                        if (data.length > 0) {
                            ...
                            $.each(data, function (i, ch) {
                                ...
                                filterChapter.append(`<option value="${ch.id}" ${selected}>${ch.title}</option>`);
                            });
                        } else {
        ...
        function loadQuestionForVerification() {
            if (currentQuestionIndex >= pendingQuestions.length) {
                ...
                loadPendingQuestions({{request('subject_id')}});
                return;
            }
            ...
            $.get("{{ url('/user/questions-bank/questions') }}/" + questionId + "/preview", function(response) {
                ...
                $('#questionPreview').html('<div class="alert alert-danger">Error loading question.</div>');
            });
        }

        // Verification actions
        $('#approveQuestionBtn').click(() => submitVerification('approved'));
        $('#rejectQuestionBtn').click(() => submitVerification('rejected'));
        $('#skipQuestionBtn').click(() => {
            currentQuestionIndex++;
            loadQuestionForVerification();
        });

        // Submit verification and reload page
    ...
    $.post("{{ route('admin.exams.questions.verify') }}", formData, function(response) {
        ...
        showAlert('danger', 'Error submitting verification.');
    }).always(function() {
        ...
        $('.selectChapter').on('change', function() {
            ...
            }).done(() => showAlert('success', 'Chapter updated successfully!'))
              .fail(() => showAlert('danger', 'Error updating chapter.'));
        });
        ...
        $.get("{{ url('user/chapters/by-subject') }}/" + "{{ request('subject_id') }}", function (data) {
            $('#filterChapter').empty().append('<option value="">-- All Chapters --</option>');
            $.each(data, function (i, ch) {
                let selected = ch.id == "{{ request('chapter_id') }}" ? 'selected' : '';
                $('#filterChapter').append(`<option value="${ch.id}" ${selected}>${ch.title}</option>`);
            });
