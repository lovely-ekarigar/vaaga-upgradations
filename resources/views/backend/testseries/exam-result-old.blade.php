        ...
        <div class="question-item">
          <div class="flex justify-between items-start">
            ...
            <div class="ml-1 flex-shrink-0 text-[10px]">
              @php
                ...
                $correctMapped = ($correctAnswer !== null) ? ($optionMap[$correctAnswer] ?? null) : null;
              @endphp
              @if(is_null($userAnswer))
                <span class="px-1.5 py-0.5 rounded-full bg-yellow-100 text-yellow-800">Not Attempted</span>
              @elseif(!is_null($correctMapped) && $userAnswer === $correctMapped)
          ...
          <div class="mt-1 space-y-1">
            ...
            @foreach($opts as $key => $opt)
              @php
                $mappedKey = $optionMap[$key] ?? null;
                $isUser = !is_null($mappedKey) && $userAnswer === $mappedKey;
