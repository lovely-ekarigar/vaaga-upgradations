...
@section('title')
<title>Mock Test Series | {{ env('APP_NAME') }}</title>
@stop
        ...
        <div class="mb-6 pb-6">
            ...
            @if($mockSeries->count() > 0)
                <div class="test-series-list">
                    @foreach($mockSeries as $ms)
                        <div class="test-series-card">
                            <div class="test-series-content">
                                <div class="d-flex flex-column flex-md-row justify-content-between">
                                    <div class="flex-grow-1 mb-3 mb-md-0">
                                        <h5 class="course-title">{{ $ms->mockSeries->course->title ?? 'Course Name' }}</h5>
                                        <p class="test-series-name mb-2">
                                            {{ $ms->mockSeries->name ?? 'N/A' }}
                                        </p>
                                        <p class="test-count mb-2">
                                            <strong>Number of Mocks:</strong> {{ $ms->mockSeries->total_test ?? 0 }}
                                        </p>
                                        @php
                                            // Use batch end date for validity - with null safety checks
                                            $validTill = null;
                                            if ($ms->batch && $ms->batch->end_date) {
                                                $validTill = \Carbon\Carbon::parse($ms->batch->end_date);
                                            }
                                            $isActive = $validTill ? $validTill->isFuture() : false;
            ...
            @else
                <div class="alert alert-info text-center">
                    <i class="bi bi-info-circle me-2"></i> You have no active mock test series.
                </div>
            @endif
        </div>
    </div>
