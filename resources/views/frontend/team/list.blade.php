...
@section('title')
<title>Our Teams | {{env('APP_NAME')}}</title>
...
<main>
   ...
   <section class="bg-primary effect-section page-heading-pad mtp-20 m-0" style="background-image: url({{asset('newassets/img/bg/bg-page-header.jpeg')}}); background-size: cover;background-position: center;background-position-y: top;">
      ...
      <div class="container position-relative">
         <div class="row">
            <div class="col-lg-8">
               <h1 class="text-white h1 ">Our Teams</h1>
   ...
   <section id="blog-item" class="py-6 section" style="background-color:#f9f9f9 !important">
        <div class="container">
            <div class="row pt-5 g-4">
                ...
                <!--                <a href="{{route('frontend.teamDeatails',['slug'=> $item->slug])}}">-->
                <!--                    <img class="card-img-top" src="/{{$item->image}}" style="height: 250px;" title="{{$item->name}}" alt="{{$item->name}}">-->
                <!--                </a> -->
                <!--                <div class="card-body p-3">-->
                <!--                    <h6 class="text-primary fw-500 mb-3 d-flex text-uppercase fs-xs letter-spacing-1"> {{$item->designation}}</h6>-->
                <!--                    <h3 class="h5 mb-3">-->
                                        <!--<a class="text-reset" href="#">{{$item->name}}</a>-->
                <!--                        <a class="text-reset" href="{{route('frontend.teamDeatails',['slug'=> $item->slug])}}">{{$item->name}}</a>-->
                <!--                    </h3>-->
                ...
                <!--        @endforeach-->
                 @foreach($teams as $item)
              <div class="col-sm-12 col-md-4 col-lg-4 text-center cursor-pointer col-12 pt-4">
                    <a href="{{route('frontend.teamDeatails',['slug'=> $item->slug])}}" contenteditable="false" style="cursor: pointer;">
                        <div class="card shadow-xs">
                          <div class="pt-2">
                                <img class="rounded-circle" style="height: 230px;
    width: 230px;" src="/{{$item->image}}" alt="{{$item->name}}" title="{{$item->name}}">
                          </div>
                            <div class="video-blog-content custom-team-box pb-3">
                                <h6 class="pt-2">{{$item->designation}}</h6>
                                <h5>{{$item->name}}</h5>
                            </div>
                        </div>
                    </a>
